...
describe('programa CRO — fase 1', () => {
  ...
  it('presenta AgentesVA como servicio antes que como directorio', () => {
    ...
    expect(header).toContain("['Services','/services/']");
    expect(header).toContain("['Servicios','/servicios/']");
    expect(footer).toContain('Automatización con IA para pequeños negocios');
  ...
  it('promete tres minutos de forma consistente en la ruta al diagnóstico', () => {
    const home = read('src/components/SpanishHome.astro');
  ...
  it('protege los mensajes verificables que sustituyen la cifra provisional', () => {
    ...
    expect(home).toContain('AUTOMATION_AREAS.map');
  ...
  it('mantiene el H1 del hero visible desde el primer render', () => {
    ...
    expect(heroHeading).toContain('Diseñamos la tecnología<br /><span>que hace avanzar tu negocio.</span>');
