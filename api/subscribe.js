...
// Migrated from api/subscribe.php
...
export default async function handler(req, res) {
  ...
  const listId = parseInt(process.env.BREVO_LIST_ID || '8', 10);
  ...
  const payload = { email: cleanEmail, listIds: [listId], updateEnabled: true };
  if (cleanName) {
    const [firstName, ...rest] = cleanName.split(' ');
    payload.attributes = { FIRSTNAME: firstName, LASTNAME: rest.join(' ') };
  }
