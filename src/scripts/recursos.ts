...
function setupFilter() {
  const search = document.getElementById('recurso-search') as HTMLInputElement | null;
  const grid = document.getElementById('recurso-grid');
  const empty = document.getElementById('recurso-empty');
  const countEl = document.getElementById('recurso-count');
  const cards = Array.from(document.querySelectorAll<HTMLElement>('.recurso-card'));
  if (!grid) return;
