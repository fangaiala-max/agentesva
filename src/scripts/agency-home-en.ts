...
import { DEMO_MODES, DEMO_MODE_KEYS, type DemoMode, type DemoExample } from '../data/automation-demo-en';
...
export function setupAutomationDemo(root: HTMLElement): Cleanup {
  ...
  const paintSteps = (current: number, finished: boolean) => {
    root.querySelectorAll<HTMLElement>('[data-demo-steps] li').forEach((item, i) => {
      ...
      label.textContent = finished && i === 3 ? sample().status : item.dataset.stepState === 'done' ? 'Complete' : item.dataset.stepState === 'active' ? 'In progress…' : 'Waiting';
    });
  };
  const sequence = createSequence(root, 4, (step, finished) => {
    ...
    run.textContent = finished ? 'Replay demo ↻' : 'Running…';
    text('[data-demo-result-state]', finished ? sample().status : `Step ${step + 1} of 4`);
    text('[data-demo-result]', finished ? sample().result : 'The example is moving through the workflow. The result will appear here.');
    status.textContent = finished ? `Demo complete. ${sample().status}. ${sample().handoff}` : `Step ${step + 1} of 4: ${DEMO_MODES[mode].steps[step]}.`;
  });
  const render = () => {
    ...
    list.replaceChildren(...content.steps.map((name, i) => {
      ...
      const small = document.createElement('small'); small.dataset.stepStatus = ''; small.textContent = 'Waiting'; div.append(strong, small); li.append(node, div); return li;
    }));
    ...
    text('[data-demo-result-state]', 'Ready to run'); text('[data-demo-result]', 'Run the example to see the result and the next step.'); text('[data-demo-handoff]', data.handoff);
    ...
    status.textContent = `Example ready: ${data.label}. Select Run the demo.`;
  };
...
function setupPreview(root: HTMLElement): Cleanup {
  ...
  const phases = ['Message received.', 'Request classified.', 'CRM record updated.', 'Human review pending.'];
  const status = root.querySelector<HTMLElement>('[data-preview-status]')!;
  const sequence = createSequence(root, 4, (step, finished) => {
    root.dataset.phase = String(step);
    root.querySelector<HTMLElement>('[data-preview-classification]')!.textContent = step >= 1 ? 'Order status' : 'Awaiting classification';
    root.querySelector<HTMLElement>('[data-preview-record]')!.textContent = step >= 2 ? 'Updated' : 'Waiting for a message';
    status.textContent = finished ? 'Final example: message classified, CRM updated, and human review pending.' : phases[step];
...
function setupIntegrations(root: HTMLElement): Cleanup {
  ...
  const sync = () => {
    ...
    button.textContent = staticMode ? 'Static view' : paused ? 'Resume movement' : 'Pause movement';
  };
